...
@constraint(m, con1,c0+c1*x_i+c2*y_i+c3*theta_i+c4*x_i*y_i+c5*x_i*theta_i+c6*y_i*theta_i+c7*x_i^2+c8*y_i^2+c9*theta_i^2<=b-small_epsilon)

#con2=zeros(Nbr_obstacle,1)
...
println(getBasisBoundsCplex(con1))

println(getBasisBoundsCplex(con2))
